...
export function FdeWorkroom({ run }: { run: WorkroomRun }) {
  ...
  return <div className="fde-workroom">
    ...
    <section className="approval-room"><div><p className="eyebrow">Approval 01</p><h2>Allow the bounded code change?</h2><p>No code, migration, credential, or infrastructure state changes before your approval.</p></div><button className="button button--accent" onClick={() => setExecutionApproved(true)} data-testid="approve-execution">{executionApproved ? "Execution approved" : "Approve execution"}</button></section>
    <section className={executionApproved ? "verification-report" : "verification-report verification-report--locked"}><p className="eyebrow">Verification report</p><h2>{executionApproved ? "The change is ready for deployment review." : "Waiting for execution approval."}</h2><div className="check-row"><span>Formatting</span><b>{executionApproved ? "Passed" : "Pending"}</b></div><div className="check-row"><span>Types and tests</span><b>{executionApproved ? "Passed" : "Pending"}</b></div><div className="check-row"><span>Build and browser review</span><b>{executionApproved ? "Passed" : "Pending"}</b></div></section>
    <section className="approval-room"><div><p className="eyebrow">Approval 02</p><h2>Deploy the verified revision?</h2><p>Production deployment is a separate founder decision with a recorded rollback path.</p></div><button className="button button--accent" disabled={!executionApproved} onClick={() => setDeploymentApproved(true)} data-testid="approve-deployment">{deploymentApproved ? "Deployment approved" : "Approve deployment"}</button></section>
